...
function HealthCard({ item }: { item: Item }) {
  ...
  return (
    <Link
      href={`/peron/${item.peronId}?tab=kesehatan`}
...
/** Segmen "Kesehatan" — overview lama /peron/kesehatan (badge + list + collapsible). */
export async function KesehatanView() {
  ...
  return (
    <div className="space-y-5">
      {/* Meta + tombol perbarui */}
      <div className="flex items-center justify-between gap-3 px-1">
        <p className="text-[13px] text-stone-400 dark:text-zinc-500">Diperbarui {relativeTime(lastUpdated)}</p>
        <RefreshButton />
      </div>
