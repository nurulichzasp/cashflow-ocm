...
---
*Snapshot dibuat: 13 Juni 2026. Diperbarui 19 Juni 2026: peron "Umum" kelebihan 50→30 (untung CV ≥90, semua produk; via `keuntunganPerKg` 70→90) + nota keterangan replas thermal disatukan (`notaKeteranganReplas`, derive saat render, "Total 0 Replas" tak muncul). Diperbarui 22 Juni 2026: tanggal replas pindah ke tiap baris RINCIAN TONASE (format pendek `formatRentangKotak`) & ringkasan `buildKeteranganReplas` jadi "Total N Replas" tanpa rentang — cegah keterangan kepanjangan/kepotong di nota Thermer kertas sempit (58mm); layout seragam A5/thermal/Thermer/gambar.*

*Lanjutan 22 Juni 2026 (sore) — nota replas final: baris tanggal kini **TANGGAL-SAJA** (kata "Replas" dibuang dari baris tanggal; ringkasan bawah tetap "Total N Replas"). Data terverifikasi **per-baris (Cabang A)** — render keempat mode (A5/thermal preview/Thermer/gambar) memang SUDAH per-baris (`details.map`/`flatMap`, guard `d.tanggalReplas`), **Koreksi akar masalah (fix lanjutan, commit 359dfa7 + berikutnya):** baris tanggal yang HILANG di Thermer ternyata adalah baris ber-**rentang tanggal** — `formatRentangKotak` memakai **en-dash "–" (U+2013)** dan **Thermer men-DROP seluruh baris ber-karakter non-ASCII** (tanggal tunggal ASCII tetap tampil). Fix: `txt()` melipat en/em-dash → hyphen ASCII (`13-15 Jun`). **Sekaligus** tanggal kini **SEBARIS dengan subtotal** (kiri=tanggal, kanan=Rp, via `padLR` di Thermer / `pair` di thermal-preview & gambar) → tiap baris = 2 baris (bukan 3); A5 tetap sub-row. `public/sw.js` VERSION dinaikkan bertahap v2→v3→**v4** (bust cache tiap rilis nota). Catatan staleness: snapshot ini berbasis HEAD `3319ecd`; sejak itu landing pula **audit menyeluruh 22 Jun** (commit `5358e5a`): gating delete kini SERAGAM owner-only, `createTransaksiKas` tolak `refTabel` dari FormData, layout owner-guard `/pengaturan`, `BETTER_AUTH_SECRET` fail-closed, hapus anak eksplisit saat delete (FK cascade OFF di Turso), `pembelian.keuntungan_per_kg` snapshot, validasi uang `.int()`. **R4 (dugaan drift tipe real→integer) = FALSE ALARM** — `PRAGMA` DB live: kolom uang sudah `integer`.*
